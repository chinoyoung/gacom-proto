...
interface V1ReviewSummaryProps {
  ...
  ctaLabel?: string;
  /**
   * Whether the histogram rows are clickable star filters.
   * Defaults to true (program detail page behavior). When false, rows
   * render as static, non-interactive markup and the "Clear filter"
   * affordance is hidden (provider page behavior).
   */
  interactive?: boolean;
}
...
export default function V1ReviewSummary({
  ...
  return (
    <div className="border border-slate-200 rounded-md bg-white p-5 sm:p-6">
          ...
          <div>
            ...
            >
              {([5, 4, 3, 2, 1] as const).map((star) => {
                ...
                // Star label + bar + count markup, shared between the
                // interactive <button> row and the static <div> row so the
                // two variants never drift apart.
                const rowContent = (
                  <>
                    {/* Star label */}
                      ...
                      <div
                        className={
                          interactive
                            ...
                            : "bg-sun-500 h-full rounded-full transition-all"
                        }
                        style={{ width: `${pct}%` }}
                      />
                    </div>
                ...
                if (!interactive) {
                  return (
                    <div key={star} role="listitem">
                      <div className="flex w-full items-center gap-2.5 rounded-md px-2 py-1">
                        {rowContent}
                      </div>
                    </div>
                  );
      ...
      <div className="mt-6 flex flex-col items-start gap-4 border-t border-slate-100 pt-5 sm:flex-row sm:items-center sm:justify-between">
        <p className="text-sm text-slate-500">
          Studied with {provider}?
        </p>
        <button
          type="button"
          className="bg-roman-500 text-white font-semibold px-5 py-2.5 rounded-md hover:bg-roman-600 cursor-pointer transition-colors text-sm focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-roman-500 focus-visible:ring-offset-1 w-full sm:w-auto"
        >
          {ctaLabel}
        </button>
      </div>
