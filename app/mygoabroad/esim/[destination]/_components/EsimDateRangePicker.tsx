...
interface EsimDateRangePickerProps {
...
export default function EsimDateRangePicker({
  startValue,
  endValue,
  min,
  onChange,
}: EsimDateRangePickerProps) {
  const [open, setOpen] = useState(false);
  // Tracks the in-session selection. When undefined, the calendar visually
  // shows the committed range from props so the user sees what they picked
  // last time. Once the user clicks, draft owns what's shown.
  ...
  useEffect(() => {
    ...
    // Two months side-by-side need ~560px of popover width with our cell sizing.
    // Drop to a single month on narrower viewports where the side-by-side layout
    // would overflow.
  ...
  function handleSelect(_range: DateRange | undefined, triggerDate: Date) {
    ...
    // Fresh session (no draft yet) OR a previously completed range: any new
    // click starts a brand-new selection from this date. We ignore RDP's range
    // computation and own the logic via triggerDate.
    if (!draft || (draft.from && draft.to)) {
      setDraft({ from: triggerDate, to: undefined });
      return;
    }

    // Mid-selection: complete the range, sorting endpoints.
    if (draft.from) {
      ...
      // Keep the picker open so the user can see the selected range; they
      // close it via the Done button, Escape, or clicking outside.
    }
  }
